...
st.set_page_config(page_title="ARYA Memory", page_icon="🧠", layout="wide")

st.markdown("## 🧠 ARYA Memory Manager")
st.caption("View, add, and remove what ARYA remembers about you")
st.markdown("---")
...
if search_query:
    ...
    if "No relevant memories" in results:
        ...
        for line in results.split("\n"):
            if line.strip():
                st.markdown(
                    f'<div style="background:rgba(102,126,234,0.1);border-left:3px solid #667eea;'
                    f'padding:8px 12px;border-radius:0 8px 8px 0;margin:4px 0;font-size:0.85rem">{line}</div>',
...
with col_a:
    new_memory = st.text_input("What should ARYA remember?", placeholder="I prefer responses in bullet points")
with col_b:
...
st.markdown("---")
st.markdown("### 🗑️ Remove a Memory")
forget_query = st.text_input("What should ARYA forget?", placeholder="my old job")
