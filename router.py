...
Decide which agent handles this."""
    ...
    try:
        ...
        print(f"  → {AGENTS[selected]['emoji']} {selected}: {reason}")
...
def compose_response(state: ARYAState) -> dict:
    ...
    final = agent_output if len(agent_output) > 50 else f"I wasn't able to find a good answer. {agent_output}"
...
if __name__ == "__main__":
    ...
    for msg in test_messages:
        ...
        print(f"\n'{msg[:50]}...'")
        print(f"  → {emoji} {agent}: {result.get('routing_reason', '')[:60]}")